...
/**
 * สถานะที่ถือว่า "ต้องตรวจ" = ยังไม่นับเป็นยอดขาย (แต่ยังนับจำนวนออเดอร์)
 *   0 / 17 = ออเดอร์ใหม่ที่ยังไม่ยืนยัน
 *   11     = รอสินค้า — ของยังไม่ได้ส่ง พอสินค้ามาแล้วเปลี่ยนเป็นยืนยันแล้ว จะกลับมานับเอง
 *            (พีสั่ง 2026-09-18 ให้ตรงเว็บ ADS SUMMARY ของทีมแอด ซึ่งนับเฉพาะออเดอร์ยืนยันแล้ว —
 *             ตรวจแล้ว UN6 17 ก.ย. ยืนยันแล้ว 142 ใบ ฿61,380 ตรงการ์ดเขาเป๊ะ ส่วนเกิน ฿390 คือใบรอสินค้า)
 * ⚠️ ใช้เฉพาะหน้า Sales + งานเตือนยูนิตขาดทุน + invariants — หน้า Admin Performance / Admin Management /
 *    Content & Ads / Report ตัดแค่ EXCLUDED_STATUSES (ยังนับออเดอร์กลุ่มนี้เป็นยอด) มาตั้งแต่ก่อนหน้านี้
 */
...
/**
 * ออเดอร์จริงใน Pancake เริ่ม 23 พ.ค. 2569 — ก่อนหน้านั้น "ยอด ฿0" แปลว่าไม่มีข้อมูล ไม่ใช่ขายไม่ได้
 * จอไหนให้เลือกย้อนหลังได้ ต้องกันช่วงก่อนวันนี้ หรือบอกให้ชัดว่าไม่ใช่ผลงานจริง
 */
export const DATA_START_YMD = '2026-05-23';
